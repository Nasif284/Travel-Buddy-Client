...
import { CalendarIcon, CloseIcon, LocationIcon, MapIcon, PersonAddIcon, PersonIcon, SparkleIcon, StarFilled, StarOutline } from "@/src/assets/icons";
import { RequestData } from "../interfaces/profile.interface";
...
export default function MatcProfile() {
  const [activeTab, setActiveTab] = useState<Tab>("about");
  const [showBuddyRequestModal, setShowBuddyRequestModal] = useState(false);
  const [bannerDismissed, setBannerDismissed] = useState(false);
  const params = useParams();
  const { data, isLoading } = useGetMatchProfile(params.matchId as string);
  const user = data?.data?.user;
  const match = data?.data?.match;
  const trip = data?.data?.matchedTrip;
  ...
  const requestSent = requestesRes?.data?.requests?.some((e:RequestData) => e.senderId === user?.id || e.receiverId === user?.id) ?? false;
  ...
  if (isLoading || loading) {
    ...
    return (
      <>
        <main className="ml-64 min-h-screen pb-32">
            ...
            {activeTab === "about" && (
              <section className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-12">
                ...
                <div className="md:col-span-2 space-y-10">
                  ...
                  <section className="mt-8 bg-white rounded-3xl border border-[#e5ebe8] p-8">
          ...
          border-[7px] border-[#0f6e56]
          flex items-center justify-center
          bg-[#eef8f4]
        "
                        >
                          <span className="text-4xl font-black text-[#0f6e56]">{match.totalPoints}%</span>
                        </div>

                        <p className="mt-3 font-bold text-[#181d1a]">Compatibility Score</p>
                      </div>

                      {/* Reasons */}
                      <div className="flex-1">
                        <h2 className="text-2xl font-black mb-5">Why you match</h2>

                        <div className="flex flex-col">
                          {match.explanation.reasons.map((reason: string) => (
                            <div
                              key={reason}
                              className="
            ...
            "
                            >
                              <div className=" text-[#0f6e56]">✓</div>

                              <p className="font-medium text-[12px] text-[#181d1a]">{reason}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <section className="mt-8">
                    <h2 className="text-2xl font-black mb-5">Matched Trip</h2>
    ...
    "
                    >
                      <div className="h-60">
                        <img src={trip.destination.coverUrl} className="w-full h-full object-cover" />
                      </div>

                      <div className="p-6">
                        <h3 className="text-3xl font-black text-[#0f6e56]">{trip.name}</h3>

                        <div className="grid md:grid-cols-2 gap-4 mt-5">
                          <InfoCard title="Destination" value={trip.destination.name} />
                          <InfoCard
                            title="Travel Dates"
                            value={`${new Date(trip.dateFrom).toLocaleDateString("en-US", {
                              day: "2-digit",
                              month: "short",
                          ...
                          <InfoCard title="Travel Style" value={trip.travelStyle} />

                          <InfoCard title="Budget Style" value={trip.budgetStyle} />
                        </div>
                      </div>
                    </div>
                  </section>

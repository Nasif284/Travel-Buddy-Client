            ...
            <div className="w-full flex justify-end">
                <div className="flex gap-3 max-w-3xl self-end flex-row-reverse">
                    <div className="flex flex-col gap-1 items-end">
                        ...
                        <div className="bg-[#005440] text-white p-4 rounded-2xl rounded-tr-sm text-sm leading-relaxed shadow-sm">
                            {chat.content}
                        </div>
                    </div>
                    {userAvatar ? (
                        <img src={userAvatar} alt="You" className="w-10 h-10 rounded-full object-cover flex-shrink-0 border border-[#bec9c3]/20" />
                    ) : (
                        <div className="w-10 h-10 rounded-full bg-[#0f6e56] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                            JD
                        </div>
                    )}
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                        {suggestions.map((s, i) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-semibold font-headline transition-colors ${i < 2
                                    ? "bg-[#c9eadb] text-[#4d6b5f] hover:bg-[#adcebf]"
                                    : "bg-[#e5e9e5] text-[#3f4944] hover:bg-[#e0e3e0] border border-[#bec9c3]/30"
                                    }`}
                            >
                                {s}
                            </button>

  ...
  return (
    <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
      {options.map((opt) => {
        ...
        return (
          <button key={opt.label} onClick={() => onSelect(opt.label)} className={`flex items-center gap-4 p-4 rounded-xl text-left transition-all active:scale-[0.98] ${isSelected ? "bg-white ring-2 ring-[#005440]" : "bg-[#e0e3e0] hover:bg-[#f1f4f1]"}`}>
